...
- 多轮问答验收需要本地 LLM Key，并可用 `scripts/prepare_chat_context.py` 从样例 CPT 生成 parsed/analysis JSON 后执行脚本验收。

## 下一步建议

1. 用户制作真实 MySQL + FineReport DBTableData 查询型和填报型 CPT。
2. 使用 `scripts/validate_real_db_sample.py` 跑真实 DB 样例验收。
3. 使用 `scripts/prepare_chat_context.py` 生成问答上下文，再用 `scripts/run_chat_acceptance.py` 跑多轮问答验收。
